...
| 2026-08-25 | Added corporate CA cert + Zeiss apt mirror: `make install` detects cz via chezmoi, fetches cert from `pass`, serves it during install. `late_command` installs cert and rewrites apt sources to Zeiss mirror with HTTPS. |
| 2026-08-25 | Corporate CA cert support: `make install` runs `chezmoi execute-template '{{ .cz }}'` to detect Zeiss network; if truthy, fetches `zeiss/ca/ca.crt` via `pass` and serves it on the preseed HTTP server. Preseed `late_command` installs it via `update-ca-certificates`. Staged `ca.crt` is gitignored and removed after install. |
| 2026-08-25 | Apt sources rewrite in preseed `late_command`: rewrites `debian.sources` to use `linux-mirror.zeiss.org/linux` (Zeiss internal mirror) and switches URIs from HTTP to HTTPS. Applied unconditionally — the mirror is expected to be accessible in the Zeiss network context where this is used. |
